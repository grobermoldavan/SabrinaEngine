...
DynamicArray<VkLayerProperties>         se_vk_utils_get_available_validation_layers(const struct AllocatorBindings& allocator);
DynamicArray<VkExtensionProperties>     se_vk_utils_get_available_instance_extensions(const struct AllocatorBindings& allocator);
...
SeVkSwapChainSupportDetails             se_vk_utils_create_swap_chain_support_details(VkSurfaceKHR surface, VkPhysicalDevice device, const struct AllocatorBindings& allocator);
...
DynamicArray<VkDeviceQueueCreateInfo>   se_vk_utils_get_queue_create_infos(const uint32_t* queues, size_t numQueues, const struct AllocatorBindings& allocator);
...
DynamicArray<VkPhysicalDevice>          se_vk_utils_get_available_physical_devices(VkInstance instance, const struct AllocatorBindings& allocator);
DynamicArray<VkQueueFamilyProperties>   se_vk_utils_get_physical_device_queue_family_properties(VkPhysicalDevice physicalDevice, const struct AllocatorBindings& allocator);
