...
DynamicArray<VkLayerProperties>         se_vk_utils_get_available_validation_layers(struct AllocatorBindings& allocator);
DynamicArray<VkExtensionProperties>     se_vk_utils_get_available_instance_extensions(struct AllocatorBindings& allocator);
...
SeVkSwapChainSupportDetails             se_vk_utils_create_swap_chain_support_details(VkSurfaceKHR surface, VkPhysicalDevice device, struct AllocatorBindings& allocator);
...
DynamicArray<VkDeviceQueueCreateInfo>   se_vk_utils_get_queue_create_infos(uint32_t* queues, size_t numQueues, struct AllocatorBindings& allocator);
...
DynamicArray<VkPhysicalDevice>          se_vk_utils_get_available_physical_devices(VkInstance instance, struct AllocatorBindings& allocator);
DynamicArray<VkQueueFamilyProperties>   se_vk_utils_get_physical_device_queue_family_properties(VkPhysicalDevice physicalDevice, struct AllocatorBindings& allocator);
bool                                    se_vk_utils_does_physical_device_supports_required_extensions(VkPhysicalDevice device, const char** extensions, size_t numExtensions, struct AllocatorBindings& allocator);
