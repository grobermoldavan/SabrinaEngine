...
static VkAttachmentReference* se_vk_render_pass_find_attachment_reference(VkAttachmentReference* refs, uint32_t numRefs, uint32_t attachmentIndex)
{
...
void se_vk_render_pass_construct(SeVkRenderPass* pass, SeVkRenderPassInfo* info)
{
    ...
    *pass = (SeVkRenderPass)
    {
        .object                 = (SeVkObject){ SE_VK_TYPE_PASS, g_renderPassIndex++ },
        .info                   = *info,
        .handle                 = VK_NULL_HANDLE,
        .attachmentLayoutInfos  = {0},
        .clearValues            = {0},
    };
    ...
    {
        attachmentDescriptions[numAttachments - 1] = (VkAttachmentDescription)
        {
            .flags          = 0,
    ...
    for (size_t it = 0; it < pass->info.numColorAttachments; it++)
    {
        ...
        attachmentDescriptions[it] = (VkAttachmentDescription)
        {
            .flags          = 0,
    ...
    for (size_t it = 0; it < numSubpasses; it++)
    {
        ...
        {
            subpassIntermediateData->colorAttachmentRefs[subpassIntermediateData->numColorRefs++] = (VkAttachmentReference){ ref, VK_IMAGE_LAYOUT_UNDEFINED };
            globalUsedAttachmentsMask |= 1 << ref;
            localUsedAttachmentsMask |= 1 << ref;
        }
        se_vk_rp_for_each_set_bit(subpassInfo->inputRefs, ref)
        {
            subpassIntermediateData->inputAttachmentRefs[subpassIntermediateData->numInputRefs++] = (VkAttachmentReference){ ref, VK_IMAGE_LAYOUT_UNDEFINED };
            globalUsedAttachmentsMask |= 1 << ref;
            localUsedAttachmentsMask |= 1 << ref;
        }
        SeVkGeneralBitmask usedFromResolveRefs = 0;
        SeVkGeneralBitmask usedToResolveRefs = 0;
        for (size_t refIt = 0; refIt < subpassInfo->numResolveRefs; refIt++)
        {
            ...
            subpassIntermediateData->resolveAttachmentRefs[ref.from] = (VkAttachmentReference){ ref.to, VK_IMAGE_LAYOUT_UNDEFINED };
        ...
        se_vk_rp_for_each_bit(usedFromResolveRefs, ref)
        {
            if (!se_vk_rp_is_bit_set(usedFromResolveRefs, ref))
                subpassIntermediateData->resolveAttachmentRefs[ref] = (VkAttachmentReference){ VK_ATTACHMENT_UNUSED, VK_IMAGE_LAYOUT_UNDEFINED };
        }
        //
        ...
        {
            subpassIntermediateData->depthStencilReference = (VkAttachmentReference){ numAttachments - 1, VK_IMAGE_LAYOUT_UNDEFINED };
    ...
    for (size_t attachmentIt = 0; attachmentIt < numAttachments; attachmentIt++)
    {
        ...
        for (size_t subpassIt = 0; subpassIt < numSubpasses; subpassIt++)
        {
            ...
            if (depth)
            {
                const bool isWrite = pass->info.subpasses[subpassIt].depthWrite;
                const bool isRead = pass->info.subpasses[subpassIt].depthRead;
    ...
    //
    for (size_t it = 0; it < numAttachments; it++)
    {
        pass->attachmentLayoutInfos[it] = (SeVkRenderPassAttachmentLayoutInfo)
        {
