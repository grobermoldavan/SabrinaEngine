...
struct { size_t index; SeAssetEntry* entry; } _se_asset_add_entry()
{
...
void _se_asset_remove_entry(size_t index)
{
    g_assetManager.ledger[index / SeAssetManager::LEDGER_ENTRY_SIZE_BITS] &= ~(1ull << (index % SeAssetManager::LEDGER_ENTRY_SIZE_BITS));
}

inline SeAssetHandle _se_asset_encode(SeAssetCategory::Type category, size_t index)
{
    return SeAssetHandle(category) | (SeAssetHandle(index) << 8);
}

inline struct { SeAssetCategory::Type category; size_t index; } _se_asset_decode(SeAssetHandle handle)
{
    return
    {
...
void _se_asset_ensure_capacity(size_t requiredCpuSpace, size_t requiredGpuSpace)
{
...
SeAssetHandle se_asset_add(const typename AssetType::Info& info)
{
    const auto [index, entry] = _se_asset_add_entry();
    ...
    if constexpr (!std::is_same_v<typename AssetType::Intermediate, void>)
    {
        ...
        _se_asset_ensure_capacity(requiredCpu, requiredGpu);
    ...
    return _se_asset_encode(AssetType::CATEGORY, index);
}

template<typename AssetType>
typename AssetType::Value* se_asset_access(SeAssetHandle handle)
{
    const auto [category, index] = _se_asset_decode(handle);
    ...
    else
    {
        ...
        se_assert(entry->queuePrev == nullptr);
        _se_asset_ensure_capacity(entry->cpuUsageMax, entry->gpuUsageMax);
