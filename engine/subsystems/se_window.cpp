...
struct SeWindow
{
    ...
    uint32_t                        width;
    uint32_t                        height;
    SeDynamicArray<SeCharacterInput>  characterInput;
} g_window;

LRESULT _se_win32_proc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam);
bool    _se_win32_process_mouse_input(SeWindow* window, UINT message, WPARAM wParam, LPARAM lParam);
bool    _se_win32_process_keyboard_input(SeWindow* window, UINT message, WPARAM wParam, LPARAM lParam);

template<typename T>
inline T se_win_get_width()
{
    return T(g_window.width);
}

template<typename T>
inline T se_win_get_height()
{
    return T(g_window.height);
}

inline void* se_win_get_native_handle()
{
    return g_window.handle;
}

inline SeMousePos se_win_get_mouse_pos()
{
    return { g_window.input.mouseX, g_window.input.mouseY };
}

inline int32_t se_win_get_mouse_wheel()
{
    return g_window.input.mouseWheel;
}

inline bool se_win_is_close_button_pressed()
{
    return g_window.input.isCloseButtonPressed;
}

inline bool se_win_is_keyboard_button_pressed(SeKeyboard::Type keyFlag)
{
    return g_window.input.keyboardButtonsCurrent[keyFlag / 64] & (1ull << (keyFlag - (keyFlag / 64) * 64));
}

inline bool se_win_is_keyboard_button_just_pressed(SeKeyboard::Type keyFlag)
{
...
inline bool se_win_is_mouse_button_pressed(SeMouse::Type keyFlag)
{
    return g_window.input.mouseButtonsCurrent & (1ull << keyFlag);
}

inline bool se_win_is_mouse_button_just_pressed(SeMouse::Type keyFlag)
{
...
inline const SeDynamicArray<SeCharacterInput>& se_win_get_character_input()
{
...
void _se_win_init(const SeSettings& settings)
{
    ...
    WNDCLASSW wc = { };
    wc.lpfnWndProc      = _se_win32_proc;
    wc.hInstance        = moduleHandle;
    wc.lpszClassName    = se_stringw_cstr(applicationNameW);
    ...
    g_window.handle = CreateWindowExW(
        ...
        se_stringw_cstr(applicationNameW),    // Window class
        se_stringw_cstr(applicationNameW),    // Window text
    ...
    se_dynamic_array_construct(g_window.characterInput, se_allocator_persistent(), 64);
...
void _se_win_update()
{
    se_dynamic_array_reset(g_window.characterInput);
...
{
    se_dynamic_array_destroy(g_window.characterInput);
...
LRESULT _se_win32_proc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    if (window)
    {
        if (_se_win32_process_mouse_input(window, uMsg, wParam, lParam))
        {
            return 0;
        }
        if (_se_win32_process_keyboard_input(window, uMsg, wParam, lParam))
        {
            return 0;
        }
        switch (uMsg)
        ...
        case WM_CHAR:
            ...
            const SeUtf32Char character = se_unicode_to_utf32(w);
            // Special characters are processed in win32_window_process_keyboard_input
            if (!se_unicode_is_special_character(character))
            {
                se_dynamic_array_push(g_window.characterInput, { .type = SeCharacterInput::CHARACTER, .character = character  });
            }
            return 0;
        }
    }
    ...
    switch (message)
    {
        case WM_KEYDOWN:
        {
            ...
            switch (SeKeyboard::Type keyboardFlag = SeKeyboard::Type(flag))
            {
                ...
                case SeKeyboard::END:
                    se_dynamic_array_push(g_window.characterInput, { .type = SeCharacterInput::SPECIAL, .special = keyboardFlag });
            }
